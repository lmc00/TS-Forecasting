...
if __name__ == "__main__":
    spark = SparkSession.builder.appName("Training Sarimax").getOrCreate()
    ...
    sarimax_model = SARIMAX(
        consumption_train[: -48 * 7].total_average_power_consumption_kW_chillers,
        order=(48, 0, 3),
        trend="c",
    ).fit()
    with open(
        data_path + "/06_models/sarimax_consumption_48_0_3.pickle", "wb"
    ) as f:  # For some reason, models in statsmodels.tsa.statespace have pending the implementation of .load()
